...
namespace SkyApm.Diagnostics.MongoDB
{
    public class MongoDiagnosticsProcessor : ITracingDiagnosticProcessor
    {
        ...
        private static readonly ConcurrentDictionary<long, SegmentContext> _segmentContextMap = new ConcurrentDictionary<long, SegmentContext>();
        ...
        public void BeforeExecuteCommand([Object] CommandStartedEvent @event)
        {
            ...
            _segmentContextMap.TryAdd((@event.OperationId ?? 0) + @event.RequestId, _contextAccessor.Context);
        }
        ...
        { 
            ...
            if (context == null) _segmentContextMap.TryRemove((@event.OperationId ?? 0) + @event.RequestId, out context);
